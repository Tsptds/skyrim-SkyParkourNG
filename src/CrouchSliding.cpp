...
namespace CrouchSliding
{
    ...
    bool IsSlideActiveFor(RE::Actor *actor, bool &out_isRoll, bool isHoldingKey)
    {
        ...
        if (ModSettings::Land_Rolling_Enabled && midair && fallTime >= 0.5f)
        {
            const auto downRay = [actor] {
                ...
                RayCastResult ray = HavokUtil::RayCast(startPos, downDir, dist, COL_LAYER_EXTEND::kCrouchSlideDistCheck, actor);

                [[unlikely]] if (ModSettings::_Debug_Enabled) { ray.Debug_Visualize(ray.didHit ? COLOR_HEX_G : COLOR_HEX_R); }
                return ray;
            }();
            ...
            if (downRay.didHit)
            {
                ...
                if (ModSettings::Enable_Stamina_Consumption && !ParkourUtility::ActorHasEnoughStamina(actor))
                {
                    ...
                    actor->UpdateRegenDelay(RE::ActorValue::kStamina, 3.0f);
    ...
    void OnStartStop(bool isStop, RE::Actor *actor, bool isRoll)
    {
        if (isStop)
        {
            ...
            if (RuntimeVariables::RecoveryFramesActive) RuntimeVariables::RecoveryFramesActive = false;
